...
function renderMessages(messages) {
    ...
    container.innerHTML = messages.map(function(m) {
        // Determinar si es mensaje enviado por admin o recibido del usuario
        const isAdminMessage = m.senderRole === 'admin' || m.senderRole === 'depositor' || m.senderRole === 'withdrawer';
        
        // Mostrar nombre del remitente para mensajes de admin
        let senderNameHtml = '';
        if (isAdminMessage && m.senderUsername) {
            const roleLabel = m.senderRole === 'admin' ? 'Admin' : 
                             m.senderRole === 'depositor' ? 'Cargas' : 'Pagos';
            senderNameHtml = '<div class="message-sender" style="font-size: 10px; color: #00ff88; margin-bottom: 4px; font-weight: bold;">' + 
                             escapeHtml(roleLabel + ': ' + m.senderUsername) + '</div>';
        }
        
        return '<div class="message ' + (isAdminMessage ? 'sent' : 'received') + '">' +
            senderNameHtml +
            (m.type === 'image' ? '<img src="' + m.content + '" alt="Imagen" onclick="openLightbox(\'' + m.content + '\')">' : '<div>' + escapeHtml(m.content) + '</div>') +
...
// Función para enviar mensaje desde el admin a un usuario específico
async function sendAdminMessage(userId, content) {
    ...
    try {
        ...
        if (response.ok) {
            // Si es el usuario seleccionado actualmente, recargar mensajes
            ...
            if (bonusAmount > 0 && selectedBonusPercent > 0) {
                ...
                try {
                    ...
                    if (bonusResponse.ok && bonusData.success) {
                        ...
                        // Solo mostrar UN toast con el total
                        showToast(`Depósito: $${amount.toLocaleString()} + Bonus: $${bonusAmount.toLocaleString()} = Total: $${totalAmount.toLocaleString()}`, 'success');
                        ...
                        // Enviar UN SOLO mensaje profesional al chat del usuario con TODO el detalle
                        const targetUser = users.find(u => u.id === selectedUserId);
                        const depositMessage = `💰 ¡Fichas cargadas! $${totalAmount.toLocaleString()}\n\n📊 Detalle:\n• Depósito: $${amount.toLocaleString()}\n• Bonus (${selectedBonusPercent}%): $${bonusAmount.toLocaleString()}\n• Total: $${totalAmount.toLocaleString()}\n\n💰 Saldo actual: $${bonusData.newBalance.toLocaleString()}\n\n✅ ¡Ya tenés tu carga en la plataforma! 🍀\n\n👤 Tu usuario: ${targetUser?.username || ''}\n🌐 Plataforma: www.jugaygana.bet\n\n¡Mucha suerte! 🎰✨`;
                        await sendSystemMessageToUser(selectedUserId, depositMessage);
                    } else {
                        // Bonus falló - mostrar solo el depósito
                        showToast(`Depósito: $${amount.toLocaleString()} (Bonus falló)`, 'warning');
                        
                        // Enviar mensaje con solo el depósito
                        const targetUser = users.find(u => u.id === selectedUserId);
                        const depositMessage = `💰 ¡Fichas cargadas! $${amount.toLocaleString()}\n\n📊 Detalle:\n• Depósito: $${amount.toLocaleString()}\n• Bonus: No aplicado\n\n💰 Saldo actual: $${newBalance.toLocaleString()}\n\n✅ ¡Ya tenés tu carga en la plataforma! 🍀\n\n👤 Tu usuario: ${targetUser?.username || ''}\n🌐 Plataforma: www.jugaygana.bet\n\n¡Mucha suerte! 🎰✨`;
                        await sendSystemMessageToUser(selectedUserId, depositMessage);
                    }
                } catch (bonusError) {
                    // Error en bonus - mostrar solo el depósito
                    showToast(`Depósito: $${amount.toLocaleString()} (Bonus no aplicado)`, 'warning');
...
function updateTransactionSummary(summary) {
    ...
    if (netBalance) {
        // Neto = Depósitos - Comisión - Retiros (sin incluir bonificaciones ni reembolsos)
        const net = (summary.deposits || 0) - commissionAmount - (summary.withdrawals || 0);
        netBalance.textContent = '$' + net.toLocaleString();
        netBalance.style.color = net >= 0 ? '#00ff88' : '#ff4444';
    }
    
    // Mostrar el total general (Depósitos + Bonificaciones - Retiros)
    if (totalGeneral) {
        const total = (summary.deposits || 0) + (summary.bonuses || 0) - (summary.withdrawals || 0);
        totalGeneral.textContent = '$' + total.toLocaleString();
        totalGeneral.style.color = total >= 0 ? '#00ff88' : '#ff4444';
    }
}
...
function renderRefunds(refunds) {
    ...
    tbody.innerHTML = refunds.map(function(r) {
        // Usar claimedAt o createdAt (timestamps de Mongoose) ya que no hay timestamp
        const dateField = r.claimedAt || r.createdAt || r.timestamp;
        const date = dateField ? new Date(dateField).toLocaleString('es-AR') : '-';
