...
async function selectConversation(userId, username) {
    ...
    // Resetear el hash de mensajes al cambiar de usuario para forzar renderizado
    lastMessagesHash = '';
    lastMessagesUserId = userId;
    
    // Limpiar el contenedor de mensajes inmediatamente para evitar mostrar mensajes del usuario anterior
    const container = document.getElementById('messagesContainer');
    if (container) {
        container.innerHTML = '<div style="text-align: center; padding: 40px; color: #888;">Cargando mensajes...</div>';
    }
...
function renderMessages(messages) {
    ...
    container.innerHTML = messages.map(function(m) {
        ...
        // Mostrar nombre del admin con color blanco que contraste bien con el fondo verde
        const adminName = isAdmin && m.senderUsername ? 
            '<span style="font-size: 10px; color: rgba(255,255,255,0.9); font-weight: bold; display: block; margin-bottom: 4px; text-shadow: 0 1px 2px rgba(0,0,0,0.3);">👤 ' + escapeHtml(m.senderUsername) + '</span>' : '';
...
async function handleAdminDeposit(e) {
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            // Construir mensaje con depósito y bonus claramente indicados
...
let lastMessagesUserId = null; // Para trackear de qué usuario es el hash actual
...
async function loadMessagesOptimized(userId) {
    try {
        // Si cambió el usuario, resetear el hash para forzar renderizado
        if (userId !== lastMessagesUserId) {
            lastMessagesHash = '';
            lastMessagesUserId = userId;
        }
