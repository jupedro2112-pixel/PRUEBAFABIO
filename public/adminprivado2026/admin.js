...
/saldo - Muestra el saldo del usuario
/bloquear - Bloquea al usuario
/desbloquear - Desbloquea al usuario
/bonus10 - Bonus del 10%
/bonus20 - Bonus del 20%
/bonus30 - Bonus del 30%
/bonus40 - Bonus del 40%
/bonus50 - Bonus del 50%
/bonus100 - Bonus del 100%`;
        }
    },
    '/saldo': {
        description: 'Muestra el saldo del usuario',
        action: function() {
            const user = users.find(function(u) { return u.id === selectedUserId; });
            if (user) {
                return `💰 Saldo de ${user.username}: $${(user.balance || 0).toLocaleString()}`;
            }
            return '❌ No hay usuario seleccionado';
        }
    },
    '/bloquear': {
        description: 'Bloquea al usuario',
        action: async function() {
            if (!selectedUserId) return '❌ No hay usuario seleccionado';
            await toggleUserBlock(selectedUserId, false);
    ...
    '/desbloquear': {
        ...
        action: async function() {
            if (!selectedUserId) return '❌ No hay usuario seleccionado';
            await toggleUserBlock(selectedUserId, true);
...
// Bonus commands
[10, 20, 30, 40, 50, 100].forEach(function(pct) {
    ADMIN_COMMANDS['/bonus' + pct] = {
        description: 'Bonus del ' + pct + '%',
        action: async function() {
            if (!selectedUserId) return '❌ No hay usuario seleccionado';
            return await applyBonus(selectedUserId, pct);
        }
    };
});
...
function filterChats(status) {
    ...
    // Actualizar tabs
    document.getElementById('tabOpenChats').className = status === 'open' ? 'btn btn-primary' : 'btn btn-secondary';
    document.getElementById('tabClosedChats').className = status === 'closed' ? 'btn btn-primary' : 'btn btn-secondary';
...
async function loadConversations() {
    try {
        const response = await fetch('/api/admin/chats/' + currentChatFilter, {
            headers: { 'Authorization': 'Bearer ' + currentToken }
        });
...
function renderConversations(chats) {
    ...
    if (!chats || chats.length === 0) {
        list.innerHTML = '<div style="padding: 40px; text-align: center; color: #888;">No hay chats ' + (currentChatFilter === 'open' ? 'abiertos' : 'cerrados') + '</div>';
        return;
    }
    
    list.innerHTML = chats.map(function(chat) {
        const isSelected = chat.userId === selectedUserId;
        const assignedClass = chat.assignedTo ? 'assigned-to-' + chat.assignedTo.replace(/\s+/g, '-') : '';
        
        return '<div class="conversation-item ' + (isSelected ? 'active' : '') + ' ' + assignedClass + '" data-user-id="' + chat.userId + '" onclick="selectConversation(\'' + chat.userId + '\')" style="cursor: pointer; padding: 12px; border-bottom: 1px solid rgba(212, 175, 55, 0.2); ' + (isSelected ? 'background: rgba(212, 175, 55, 0.2);' : '') + '">' +
            '<div style="display: flex; justify-content: space-between; align-items: flex-start;">' +
                '<div style="flex: 1;">' +
                    '<div style="font-weight: bold; color: #d4af37; font-size: 14px;">' + escapeHtml(chat.username) + '</div>' +
                    '<div style="font-size: 12px; color: #888; margin-top: 4px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">' + escapeHtml(chat.lastMessage?.content?.substring(0, 50) || 'Sin mensajes') + '</div>' +
                    (chat.assignedTo ? '<div style="font-size: 10px; color: #00ff88; margin-top: 4px;">👤 Asignado a: ' + escapeHtml(chat.assignedTo) + '</div>' : '') +
                '</div>' +
                '<div style="text-align: right;">' +
                    (chat.unreadCount > 0 ? '<span class="badge" style="background: #ff4444; color: #fff; padding: 2px 8px; border-radius: 10px; font-size: 11px;">' + chat.unreadCount + '</span>' : '') +
                    '<div style="font-size: 10px; color: #666; margin-top: 4px;">' + formatTime(chat.lastMessage?.timestamp) + '</div>' +
                '</div>' +
            '</div>' +
        '</div>';
    }).join('');
...
async function closeChat(userId) {
    if (!confirm('¿Cerrar este chat? El usuario podrá reabrirlo enviando un nuevo mensaje.')) return;
    ...
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById('chatUserName').textContent = 'Usuario';
            document.getElementById('chatUserBalance').textContent = 'Balance: $0';
        }
    } catch (error) {
...
function renderTransactions(transactions) {
    ...
    if (!transactions || transactions.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" style="padding: 40px; text-align: center; color: #888;">No hay transacciones para mostrar</td></tr>';
        return;
    }
...
function updateTransactionSummary(summary) {
    ...
    if (netBalance) {
        const net = (summary.deposits || 0) - (summary.withdrawals || 0) - (summary.bonuses || 0) - (summary.refunds || 0);
