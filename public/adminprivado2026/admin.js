...
async function handleAdminDeposit(e) {
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            if (bonusAmount > 0 && selectedBonusPercent > 0) {
                ...
                try {
                    ...
                    if (bonusResponse.ok && bonusData.success) {
                        ...
                        // Enviar mensaje al usuario con el total
                        await sendAdminMessage(selectedUserId, `💰 Depósito realizado: $${amount.toLocaleString()} + Bonus: $${bonusAmount.toLocaleString()} = Total: $${totalAmount.toLocaleString()}\n💰 Saldo actual: $${bonusData.newBalance.toLocaleString()}`);
                    } else {
                        showToast(`Depósito: $${amount.toLocaleString()} (Bonus falló: ${bonusData.error || 'Error'})`, 'warning');
                        await sendAdminMessage(selectedUserId, `💰 Depósito realizado: $${amount.toLocaleString()}\n💰 Saldo actual: $${newBalance.toLocaleString()}`);
                    }
                } catch (bonusError) {
            ...
            const totalAmount = amount + (bonusAmount > 0 && selectedBonusPercent > 0 ? bonusAmount : 0);
