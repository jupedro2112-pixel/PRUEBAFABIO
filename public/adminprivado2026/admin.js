...
async function loadStats() {
    try {
        ...
        if (response.ok) {
            ...
            // Actualizar estadísticas en la barra superior
            ...
            if (statUnread) statUnread.textContent = stats.unreadMessages || 0;
            
            // Actualizar estadísticas en la sección de estadísticas si existe
            const statsTotalDeposits = document.getElementById('statsTotalDeposits');
            const statsTotalWithdrawals = document.getElementById('statsTotalWithdrawals');
            const statsTotalBonuses = document.getElementById('statsTotalBonuses');
            const statsTotalRefunds = document.getElementById('statsTotalRefunds');
            const statsNetBalance = document.getElementById('statsNetBalance');
            
            if (statsTotalDeposits) statsTotalDeposits.textContent = '$' + (stats.totalDeposits || 0).toLocaleString();
            if (statsTotalWithdrawals) statsTotalWithdrawals.textContent = '$' + (stats.totalWithdrawals || 0).toLocaleString();
            if (statsTotalBonuses) statsTotalBonuses.textContent = '$' + (stats.totalBonuses || 0).toLocaleString();
            if (statsTotalRefunds) statsTotalRefunds.textContent = '$' + (stats.totalRefunds || 0).toLocaleString();
            if (statsNetBalance) statsNetBalance.textContent = '$' + (stats.netBalance || 0).toLocaleString();
        }
    } catch (error) {
...
function renderMessages(messages) {
    ...
    container.innerHTML = messages.map(function(m) {
        const isAdmin = m.senderRole === 'admin' || m.senderRole === 'depositor' || m.senderRole === 'withdrawer';
        const adminName = isAdmin && m.senderUsername ? 
            '<span style="font-size: 10px; color: #d4af37; font-weight: bold; display: block; margin-bottom: 4px;">👤 ' + escapeHtml(m.senderUsername) + '</span>' : '';
        
        return '<div class="message ' + (isAdmin ? 'sent' : 'received') + '">' +
            adminName +
            (m.type === 'image' ? '<img src="' + m.content + '" alt="Imagen" onclick="openImageLightbox(\'' + m.content + '\')" style="cursor: zoom-in;">' : '<div>' + escapeHtml(m.content) + '</div>') +
...
async function handleAdminDeposit(e) {
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            const hasBonus = bonusAmount > 0 && selectedBonusPercent > 0;
            const totalAmount = amount + (hasBonus ? bonusAmount : 0);
            
            let depositMessage = `💰 ¡Fichas cargadas!`;
            if (hasBonus) {
                depositMessage += ` $${amount.toLocaleString()} + $${bonusAmount.toLocaleString()} de BONUS (${selectedBonusPercent}%)`;
            } else {
                depositMessage += ` $${amount.toLocaleString()}`;
            }
...
function startMessagePolling() {
    ...
    // Polling ultra rápido (1 segundo) para máxima fluidez
    ...
    // Actualizar saldo del usuario seleccionado cada 5 segundos
...
// Actualizar saldo del usuario seleccionado
async function updateSelectedUserBalance() {
    try {
        const user = users.find(u => u.id === selectedUserId);
        if (!user) return;
        
        const response = await fetch('/api/admin/balance/' + encodeURIComponent(user.username), {
            headers: { 'Authorization': 'Bearer ' + currentToken }
        });
        ...
        if (response.ok) {
            ...
            if (data.balance !== undefined && data.balance !== user.balance) {
                ...
                updateUserBalanceInUI(data.balance);
...
function renderRefunds(refunds) {
    ...
    tbody.innerHTML = refunds.map(function(r) {
        const date = new Date(r.date || r.timestamp).toLocaleString('es-AR');
