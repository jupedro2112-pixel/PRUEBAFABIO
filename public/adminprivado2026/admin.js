...
async function loadConversations() {
    try {
        // Usar el endpoint específico para el estado actual (open/closed)
...
function renderTransactions(transactions) {
    ...
    tbody.innerHTML = transactions.map(function(t) {
        ...
        // Mostrar quién hizo la transacción
        const adminInfo = t.adminUsername ? `<span style="color: #d4af37; font-size: 10px;">(${escapeHtml(t.adminUsername)})</span>` : '';
        ...
        return '<tr style="border-bottom: 1px solid rgba(212, 175, 55, 0.1);">' +
            ...
            '<td style="padding: 12px; color: #888; font-size: 12px;">' + escapeHtml(t.description || '-') + ' ' + adminInfo + '</td>' +
        '</tr>';
    }).join('');
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (databaseNavItem && currentUser?.role === 'admin') {
    ...
    // PAGOS section - solo para withdrawer y admin
    const pagosNavItem = document.getElementById('pagosNavItem');
    if (pagosNavItem && (currentUser?.role === 'admin' || currentUser?.role === 'withdrawer')) {
        pagosNavItem.classList.remove('hidden');
    }
    
    // Enviar a PAGOS button
...
function configureUIBasedOnRole() {
    ...
    // Mostrar/ocultar sección PAGOS según rol
    const pagosNavItem = document.getElementById('pagosNavItem');
    if (pagosNavItem) {
        pagosNavItem.classList.toggle('hidden', role !== 'admin' && role !== 'withdrawer');
    }
    
    // Depositer: solo puede depositar, no ver PAGOS
    if (role === 'depositor') {
        // Ocultar botón de retiro
        const withdrawBtn = document.getElementById('withdrawUserBtn');
        if (withdrawBtn) withdrawBtn.style.display = 'none';
        
        // Ocultar botón de enviar a PAGOS
        const sendToPagosBtn = document.getElementById('sendToPagosBtn');
        ...
        if (createUserRole) {
            // Solo dejar opción 'user'
            createUserRole.innerHTML = '<option value="user">Usuario</option>';
        }
    }
    
    // Withdrawer: solo puede retirar, ver PAGOS
    if (role === 'withdrawer') {
        ...
        // Ocultar botón de enviar a PAGOS (ya está en PAGOS)
        const sendToPagosBtn = document.getElementById('sendToPagosBtn');
        if (sendToPagosBtn) sendToPagosBtn.style.display = 'none';
        ...
        if (chatsNavItem) {
            chatsNavItem.querySelector('span:last-child').textContent = 'Chats (solo lectura)';
        }
    }
