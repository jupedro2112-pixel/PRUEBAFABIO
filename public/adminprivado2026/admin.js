...
async function loadConversations() {
    try {
        ...
        // Usar el endpoint de conversaciones (más simple y confiable)
        ...
        if (response.ok) {
            ...
            console.log('✅ Conversaciones cargadas:', allConversations.length);
            
            // Filtrar por estado si es necesario
            if (currentChatFilter === 'open' || currentChatFilter === 'closed') {
                // Obtener estados de chats
                const statusResponse = await fetch('/api/admin/chat-status/all', {
                    headers: { 'Authorization': 'Bearer ' + currentToken }
                });
                
                let chatStatuses = {};
                if (statusResponse.ok) {
                    chatStatuses = await statusResponse.json();
                }
                
                // Filtrar conversaciones por estado
                conversations = allConversations.filter(conv => {
                    const status = chatStatuses[conv.userId]?.status || 'open';
                    return status === currentChatFilter;
                });
            } else {
...
function filterChats(status) {
    ...
    if (tabOpen) tabOpen.className = status === 'open' ? 'btn btn-primary' : 'btn btn-secondary';
    if (tabClosed) tabClosed.className = status === 'closed' ? 'btn btn-primary' : 'btn btn-secondary';
