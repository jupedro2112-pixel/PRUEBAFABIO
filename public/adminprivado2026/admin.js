...
async function closeChat(userId) {
    if (!confirm('¿Cerrar este chat? Podrás seguir enviando mensajes. El chat se reabrirá automáticamente si el usuario responde.')) return;
    ...
    try {
        ...
        if (response.ok) {
            showToast('Chat cerrado. Puedes seguir enviando mensajes.', 'success');
            // NO limpiar selectedUserId - permitir seguir enviando mensajes
            // NO limpiar el chat - mantener visible
            loadConversations();
            // Cambiar a la pestaña de chats cerrados
            filterChats('closed');
        }
    } catch (error) {
