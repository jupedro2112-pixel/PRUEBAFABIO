...
async function makeBonus(username, amount, description = '') {
  ...
  try {
    // Usar el mismo formato que makeDeposit pero con deposit_type diferente
    const body = toFormUrlEncoded({
      action: 'DepositMoney',
      token: jugaygana.SESSION_TOKEN,
      childid: userInfo.id,
      amount: Math.round(amount), // Mismo formato que makeDeposit
      currency: 'ARS',
      deposit_type: 'individual_bonus',
      description: description || 'Bonificación - Sala de Juegos'
    });
    
    console.log('🎁 Enviando request a JUGAYGANA:', { 
      action: 'DepositMoney', 
      childid: userInfo.id, 
    ...
    // Verificar respuesta exitosa (mismo formato que makeDeposit)
    if (data.success || data.status === 'success' || data.transfer_id || data.transferId) {
      return {
        success: true,
