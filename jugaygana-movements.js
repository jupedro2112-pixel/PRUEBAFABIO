...
async function makeBonus(username, amount, description = '') {
  ...
  try {
    // EXACTAMENTE IGUAL a makeDeposit, solo cambia deposit_type
    const body = toFormUrlEncoded({
      action: 'DepositMoney',
      token: jugaygana.SESSION_TOKEN,
      childid: userInfo.id,
      amount: Math.round(amount), // API espera el monto directo (igual que makeDeposit)
      currency: 'ARS',
      deposit_type: 'individual_bonus', // <-- ÚNICA DIFERENCIA con makeDeposit
