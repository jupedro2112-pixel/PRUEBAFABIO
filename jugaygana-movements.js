...
async function makeBonus(username, amount, description = '') {
  console.log('🎁 makeBonus iniciado:', { username, amount, description });
  ...
  const sessionOk = await jugaygana.ensureSession();
  console.log('🎁 Sesión válida:', sessionOk);
  if (!sessionOk) {
    return { success: false, error: 'No hay sesión válida' };
  }
  ...
  // Obtener userId del usuario (o crearlo si no existe)
  let userInfo = await jugaygana.getUserInfoByName(username);
  console.log('🎁 UserInfo obtenido:', userInfo ? { id: userInfo.id, username: userInfo.username } : null);
  ...
  try {
    const requestBody = {
      action: 'DepositMoney',
      token: jugaygana.SESSION_TOKEN,
      childid: userInfo.id.toString(),
      amount: Math.round(parseFloat(amount) * 100).toString(), // API espera el monto en centavos
      currency: 'ARS',
      deposit_type: 'individual_bonus',
      description: (description || 'Bonificación - Sala de Juegos').substring(0, 100)
    };
    
    console.log('🎁 Enviando request a JUGAYGANA:', { ...requestBody, token: '***' });
    ...
    console.log('🎁 Respuesta de JUGAYGANA:', resp.data);
    ...
    if (data.success || data.status === 'success' || data.transfer_id || data.transferId) {
      ...
      console.log('❌ Error en respuesta de JUGAYGANA:', data);
      return {
        success: false,
        error: data.error || data.message || data.msg || JSON.stringify(data) || 'Error al realizar bonificación'
      };
    }
  } catch (error) {
