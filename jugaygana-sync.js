...
const { User, SyncLog, isConnected } = require('./database');
const fs = require('fs');
const path = require('path');

// Configuración para fallback JSON
...
if (!fs.existsSync(USERS_FILE)) {
  try {
    ...
    console.error('Error creando archivo de usuarios:', error);
  }
}
if (!fs.existsSync(SYNC_LOG_FILE)) {
  try {
    ...
    console.error('Error creando archivo de sync log:', error);
  }
}

// Helpers para fallback JSON
...
function saveSyncLogJSON(log) {
  try {
    fs.writeFileSync(SYNC_LOG_FILE, JSON.stringify(log, null, 2));
  } catch (error) {
    console.error('Error guardando sync log JSON:', error);
  }
}
...
async function syncAllUsers(progressCallback = null) {
  ...
  if (isConnected()) {
    // Usar MongoDB
    console.log('💾 Usando MongoDB para sincronización');
    
    // Contar usuarios locales iniciales
    ...
    console.log(`📊 Usuarios locales final: ${finalCount}`);
    ...
    // Usar JSON fallback
    console.log('📁 Usando JSON para sincronización');
    
    let localUsers = loadUsersJSON();
...
async function syncRecentUsers(limit = 100) {
  ...
  if (isConnected()) {
    ...
    const totalLocal = await User.countDocuments();
...
async function syncSingleUser(username) {
  ...
  if (isConnected()) {
    ...
    const localUsers = loadUsersJSON();
