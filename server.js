...
app.get('/api/refunds/status', authMiddleware, async (req, res) => {
  try {
    ...
    // Calcular montos potenciales basados en movimientos reales
    const deposits = yesterdayMovements.success ? yesterdayMovements.totalDeposits : 0;
    const withdrawals = yesterdayMovements.success ? yesterdayMovements.totalWithdraws : 0;
    const netAmount = yesterdayMovements.success ? yesterdayMovements.net : 0;
    ...
    res.json({
      ...
      claimedToday: claimedToday.success ? claimedToday.claimed : false
        deposits: todayMovements.deposits.total,
        withdrawals: todayMovements.withdrawals.total
...
app.post('/api/fire/claim', authMiddleware, async (req, res) => {
  try {
    ...
    if (!hasActivityToday(userId)) {
      return res.status(400).json({ 
        error: 'Necesitas hacer un depósito o retiro hoy para reclamar el fueguito' 
      });
    }
    ...
    // Si no reclamó ayer, resetear racha
    ...
    if (userRewards.streak === 10) {
      ...
      // Acreditar recompensa en JUGAYGANA
      const depositResult = await jugayganaMovements.makeDeposit(
        username,
        reward,
      ...
      if (!depositResult.success) {
        // Si falla el depósito, no marcar como reclamado
        return res.status(400).json({ 
          error: 'Error al acreditar recompensa: ' + depositResult.error 
        });
      }
