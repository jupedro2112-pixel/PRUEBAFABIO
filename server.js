...
app.get('/api/auth/check-username', async (req, res) => {
  try {
    ...
    // Verificar en JUGAYGANA - AHORA PRIMERO CREAMOS EN JUGAYGANA
    // Si existe en JUGAYGANA, está disponible para vincular
    // Si NO existe, debemos crearlo primero en JUGAYGANA
    try {
      const jgUser = await jugaygana.getUserInfoByName(username);
      if (jgUser) {
        // El usuario existe en JUGAYGANA pero no en nuestra plataforma
        // Está disponible para vincular
        return res.json({ 
          available: true, 
          message: 'Usuario disponible para vincular con JUGAYGANA',
    ...
    // El usuario no existe en ningún lado, debe crearse primero en JUGAYGANA
    res.json({ 
      available: true, 
      message: 'Usuario disponible - se creará en JUGAYGANA primero',
...
app.post('/api/admin/send-cbu', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    const message = config.cbu.message
      .replace('{cbu}', config.cbu.number)
      .replace('{alias}', config.cbu.alias)
      .replace('{bank}', config.cbu.bank)
      .replace('{titular}', config.cbu.titular);
...
app.get('/api/admin/chats/:status', authMiddleware, adminMiddleware, (req, res) => {
  try {
    ...
    // Agrupar mensajes por usuario (solo mensajes donde el usuario es el remitente o destinatario)
    const userMessages = {};
    messages.forEach(msg => {
      // Solo considerar mensajes entre usuarios y admins/sistema
    ...
    Object.keys(userMessages).forEach(userId => {
      ...
      // Por defecto, los chats nuevos son 'open'
      const statusInfo = chatStatus[userId] || { status: 'open', assignedTo: null };
      
      console.log(`👤 ${user.username} - Estado: ${statusInfo.status} (buscando: ${status})`);
...
app.post('/api/admin/chats/:userId/close', authMiddleware, adminMiddleware, (req, res) => {
  try {
    ...
    messages.push({
      ...
      receiverId: userId,
      receiverRole: 'user',
      content: '✅ El chat ha sido cerrado. Si necesitas más ayuda, escribe un nuevo mensaje.',
      type: 'text',
      timestamp: new Date().toISOString(),
      read: false
    });
...
app.post('/api/admin/bonus', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    // Usar creditUserBalance que hace deposit_type: individual_bonus
    const result = await jugaygana.creditUserBalance(username, amount);
...
app.use('/api/admin/bonus', authMiddleware, depositorMiddleware, recordTransaction);
app.use('/api/refunds/claim', recordTransaction);
