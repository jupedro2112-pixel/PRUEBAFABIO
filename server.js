...
// Modelos de Mongoose
let User, Message, Transaction, Config;
...
if (MONGODB_URI) {
  ...
  const transactionSchema = new mongoose.Schema({
    ...
    method: String, reference: String, timestamp: String
  }, { timestamps: true });

  const configSchema = new mongoose.Schema({
    key: String, cbu: Object,
    welcomeMessage: String, depositMessage: String
  }, { timestamps: true });
...
// Middleware para silenciar logs de rutas de polling
app.use((req, res, next) => {
  // Silenciar logs de /api/messages/ (polling del chat)
  if (req.path.startsWith('/api/messages/') && req.method === 'GET') {
    // No loguear estas peticiones
  }
  next();
});
...
const saveUsers = async (users) => {
  // Guardar en archivo (backup local)
  fs.writeFileSync(USERS_FILE, JSON.stringify(users, null, 2));
  
  // Guardar en MongoDB (principal)
...
async function initializeData() {
  // ============================================
  // CARGAR USUARIOS DESDE MONGODB (PRINCIPAL)
  // ============================================
  let users = [];
    ...
    try {
      ...
      if (mongoUsers && mongoUsers.length > 0) {
        users = mongoUsers.map(u => ({
          ...
          role: u.role, accountNumber: u.accountNumber,
          balance: u.balance, bonusBalance: u.bonusBalance,
          createdAt: u.createdAt, lastLogin: u.lastLogin,
          isActive: u.isActive, tokenVersion: u.tokenVersion,
          jugayganaUserId: u.jugayganaUserId,
          jugayganaUsername: u.jugayganaUsername,
          jugayganaSyncStatus: u.jugayganaSyncStatus
        }));
        fs.writeFileSync(USERS_FILE, JSON.stringify(users, null, 2));
        console.log(`✅ ${users.length} usuarios cargados desde MongoDB`);
      } else {
        // Si no hay usuarios en MongoDB, cargar desde archivo (primera vez)
        users = loadUsers();
        console.log(`📁 ${users.length} usuarios cargados desde archivo`);
      }
    } catch (err) {
      console.log('⚠️ Error MongoDB, usando archivo:', err.message);
      users = loadUsers();
    }
  } else {
    // Sin MongoDB, usar archivo
    users = loadUsers();
  }
  ...
  if (mongoConnected && Message) {
    try {
      ...
      if (mongoMessages && mongoMessages.length > 0) {
        const cleanMessages = mongoMessages.map(m => ({
          id: m.id, senderId: m.senderId, senderUsername: m.senderUsername,
          senderRole: m.senderRole, receiverId: m.receiverId,
          receiverRole: m.receiverRole, content: m.content,
          type: m.type, timestamp: m.timestamp, read: m.read
        }));
        fs.writeFileSync(MESSAGES_FILE, JSON.stringify(cleanMessages, null, 2));
        console.log(`✅ ${mongoMessages.length} mensajes cargados desde MongoDB`);
      }
    } catch (err) {
      console.log('⚠️ Error cargando mensajes:', err.message);
    }
  }
