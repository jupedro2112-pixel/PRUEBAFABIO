...
app.get('/api/refunds/status', authMiddleware, async (req, res) => {
  try {
    ...
    // Obtener movimientos de hoy para mostrar actividad actual
    const today = new Date();
    const todayStr = today.toISOString().split('T')[0];
    const todayMovements = await jugayganaMovements.getDailyMovements(username, todayStr);
...
async function initializeData() {
  ...
  const adminExists = users.find(u => u.username === 'ignite100');
  if (!adminExists) {
