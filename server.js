...
function broadcastStats() {
  const stats = {
    ...
    totalUsers: await loadUsers().filter(u => u.role === 'user').length
  };
...
async function initializeData() {
  ...
  let adminExists;
  if (mongoose.connection.readyState === 1) {
    adminExists = await User.findOne({ username: 'ignite100' });
  } else {
    const users = await loadUsers();
    adminExists = users.find(u => u.username === 'ignite100');
  }
  ...
  if (!adminExists) {
    ...
    const newPassword = await bcrypt.hash('pepsi100', 10);
    if (mongoose.connection.readyState === 1) {
      await User.updateOne(
        { username: 'ignite100' },
        { password: newPassword, role: 'admin', isActive: true }
      );
    } else {
      const users = await loadUsers();
      const idx = users.findIndex(u => u.username === 'ignite100');
      if (idx !== -1) {
  ...
  let oldAdmin;
  if (mongoose.connection.readyState === 1) {
    oldAdmin = await User.findOne({ username: 'admin' });
  } else {
    const users = await loadUsers();
    oldAdmin = users.find(u => u.username === 'admin');
  }
  ...
  let testUser;
  if (mongoose.connection.readyState === 1) {
    testUser = await User.findOne({ username: '672rosana1' });
  } else {
    const users = await loadUsers();
    testUser = users.find(u => u.username === '672rosana1');
  }
...
function await saveConfig(config) {
  try {
    fs.writeFileSync(CONFIG_FILE, JSON.stringify(config, null, 2));
  } catch (error) {
