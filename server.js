...
const userSchema = new mongoose.Schema({
  ...
  role: String, accountNumber: String,
  balance: Number, bonusBalance: Number,
  createdAt: String, lastLogin: String,
  isActive: Boolean, tokenVersion: Number,
  jugayganaUserId: String, jugayganaUsername: String,
  jugayganaSyncStatus: String, source: String,
  lastBonusDate: String, bonusDay: Number,
  dailyWager: Number, weeklyWager: Number, monthlyWager: Number,
  lastDailyRefund: String, lastWeeklyRefund: String, lastMonthlyRefund: String
}, { timestamps: true });

const messageSchema = new mongoose.Schema({
  id: String, senderId: String, senderUsername: String,
  senderRole: String, receiverId: String, receiverRole: String,
  content: String, type: String, timestamp: String, read: Boolean
}, { timestamps: true });

// Schema para comandos personalizados
const commandSchema = new mongoose.Schema({
  id: String, name: String, description: String,
  response: String, createdBy: String, createdAt: String,
...
const transactionSchema = new mongoose.Schema({
  ...
  createdAt: String, jugayganaTransactionId: String
}, { timestamps: true });
...
const refundSchema = new mongoose.Schema({
  ...
  netAmount: Number, deposits: Number, withdrawals: Number,
  wagerAmount: Number, status: String,
  processedAt: String, createdAt: String, date: String
}, { timestamps: true });
...
// ============================================
// FUNCIONES PARA COMANDOS PERSONALIZADOS
// ============================================
...
// ============================================
// FUNCIONES PARA CONFIGURACIÓN (CBU)
// ============================================
...
// Guardar una transacción individual
...
const saveRefunds = async (refunds) => {
  fs.writeFileSync(REFUNDS_FILE, JSON.stringify(refunds, null, 2));
  if (mongoConnected) {
    try {
      await Refund.deleteMany({});
      if (refunds.length > 0) await Refund.insertMany(refunds);
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    // Generar token (sesión permanente - 100 años)
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    // Si el usuario cambió su contraseña, SOLO permitir la nueva contraseña
    // Si el usuario NUNCA cambió su contraseña, permitir 'asd123'
    let isValidPassword = await bcrypt.compare(password, user.password);
    
    // Si la contraseña almacenada no coincide y el usuario nunca cambió su contraseña
    // permitir iniciar sesión con 'asd123' (solo para usuarios que nunca cambiaron)
    ...
    // IMPORTANTE: Si el usuario YA cambió su contraseña (passwordChangedAt existe)
    // y la contraseña no coincide, DENEGAR acceso
    if (!isValidPassword) {
    ...
    // 5. Generar token (sesión permanente - 100 años)
    const token = jwt.sign(
...
app.put('/api/users/:id', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    // Solo el admin principal (ignite100) puede modificar otros admins
...
// ============================================
// MENSAJES DE BIENVENIDA (DESDE ADMIN)
// ============================================
app.post('/api/messages/welcome', authMiddleware, async (req, res) => {
  try {
    ...
    // Crear mensaje como si fuera del admin
    const message = {
      id: uuidv4(),
      senderId: 'admin',
...
async function initializeData() {
  ...
  if (mongoConnected) {
    try {
      ...
      if (mongoConfig) {
        fs.writeFileSync(SYSTEM_CONFIG_FILE, JSON.stringify(mongoConfig, null, 2));
  ...
  // ============================================
  // CARGAR REEMBOLSOS DESDE MONGODB
  // ============================================
  if (mongoConnected) {
    try {
...
async function loadConfig() {
  ...
  if (mongoConnected) {
    try {
      ...
      // Crear config por defecto en MongoDB
      await Config.create({ key: 'main', ...defaultConfig });
      return defaultConfig;
    } catch (err) {
      console.error('Error cargando config de MongoDB:', err.message);
...
// Las transacciones se registran directamente en cada endpoint
...
app.get('/api/admin/stats', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    const unreadMessages = messages.filter(m => !m.read && m.receiverRole === 'admin').length;
    ...
    // Calcular totales de reembolsos
